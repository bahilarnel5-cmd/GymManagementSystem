...
const memberNavItems = [
  ...
  { to: '/member/coaches', icon: 'bi-person-badge', label: 'Coaches' },
  { to: '/member/renewals', icon: 'bi-arrow-repeat', label: 'Renewals' },
]
        ...
        <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto">
          {memberNavItems.map((item) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-gradient-to-b ${memberIconColors[item.to]}`} />
                )}
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                  isActive
                    ? `bg-gradient-to-br ${memberIconColors[item.to]} shadow-md`
                    : 'bg-white/[0.05]'
                }`}>
